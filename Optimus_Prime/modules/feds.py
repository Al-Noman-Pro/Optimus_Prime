...
import Optimus_Prime.modules.sql.feds_sql as sql
...
from Optimus_Prime.modules.helper_funcs.alternate import send_message
from Optimus_Prime.modules.helper_funcs.chat_status import is_user_admin
from Optimus_Prime.modules.helper_funcs.extraction import (
    extract_unt_fedban,
    extract_user,
    extract_user_fban,
)
from Optimus_Prime.modules.helper_funcs.string_handling import markdown_parser
...
def fed_ban(update: Update, context: CallbackContext):
    ...
    if int(user_id) in TIGER_USERS:
        message.reply_text("TIGER_USERS cannot be fed banned!")
        return
    ...
    if args:
        if args[0] == "json":
            ...
            with BytesIO(str.encode(backups)) as output:
                output.name = "Optimus_Prime_fbanned_users.json"
                update.effective_message.reply_document(
                    document=output,
                    filename="Optimus_Prime_fbanned_users.json",
            ...
            for users in getfban:
                ...
                backups += "\n"
            with BytesIO(str.encode(backups)) as output:
                output.name = "Optimus_Prime_fbanned_users.csv"
                update.effective_message.reply_document(
                    document=output,
                    filename="Optimus_Prime_fbanned_users.csv",
