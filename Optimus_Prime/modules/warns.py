...
import telegram
from Optimus_Prime import TIGER_USERS, WHITELIST_USERS, dispatcher
from Optimus_Prime.modules.disable import DisableAbleCommandHandler
from Optimus_Prime.modules.helper_funcs.chat_status import (
    bot_admin,
    can_restrict,
    is_user_admin,
    user_admin,
    user_admin_no_reply)
from Optimus_Prime.modules.helper_funcs.extraction import (
    extract_text,
    extract_user,
    extract_user_and_text,
)
from Optimus_Prime.modules.helper_funcs.filters import CustomFilters
from Optimus_Prime.modules.helper_funcs.misc import split_message
from Optimus_Prime.modules.helper_funcs.string_handling import split_quotes
from Optimus_Prime.modules.log_channel import loggable
from Optimus_Prime.modules.sql import warns_sql as sql
...
from Optimus_Prime.modules.sql.approve_sql import is_approved
...
def warn(
    ...
    if user.id in TIGER_USERS:
        if warner:
            message.reply_text("TIGER_USERS cant be warned.")
        else:
            message.reply_text(
                "Tiger triggered an auto warn filter!\n I can't warn TIGER_USERS but they should avoid abusing this.")
        return
    ...
    if num_warns >= limit:
        ...
        # message.bot.send_sticker(chat.id, BAN_STICKER)  # Optimus_Prime's sticker
        keyboard = None
        log_reason = (
