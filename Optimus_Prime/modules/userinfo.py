...
Stats_Photo = "https://telegra.ph/file/b9b8713a3376bea56e6b6.jpg"
...
from Optimus_Prime.__main__ import STATS, TOKEN, USER_INFO
import Optimus_Prime.modules.sql.userinfo_sql as sql
from Optimus_Prime.modules.disable import DisableAbleCommandHandler
from Optimus_Prime.modules.sql.global_bans_sql import is_user_gbanned
from Optimus_Prime.modules.sql.afk_sql import is_afk, check_afk_status
from Optimus_Prime.modules.sql.users_sql import get_user_num_chats
from Optimus_Prime.modules.helper_funcs.chat_status import sudo_plus
from Optimus_Prime.modules.helper_funcs.extraction import extract_user
from Optimus_Prime import telethn as Optimus_PrimeTelethonClient
...
@Optimus_PrimeTelethonClient.on(
    events.NewMessage(
...
def info(update: Update, context: CallbackContext):
    ...
    if user.id == OWNER_ID:
        text += "\n\nThe Disaster level of this person is 'Bot Owner'."
        disaster_level_present = True
    elif user.id in DEV_USERS:
        text += "\n\nThis user is member of 'Atrocious Association."
        disaster_level_present = True
    elif user.id in SUDO_USERS:
        text += "\n\nThe Disaster level of this person is 'Sudo user'."
        disaster_level_present = True
    elif user.id in SUPPORT_USERS:
        text += "\n\nThe Disaster level of this person is 'Support User'."
        disaster_level_present = True
...
def stats(update: Update, context: CallbackContext):
    stats = "<b>📊 Optimus_Primes Current stats:</b>\n" + "\n".join([mod.__stats__() for mod in STATS])
    result = re.sub(r"(\d+)", r"<code>\1</code>", stats)
    update.effective_message.reply_photo(Stats_Photo,result, parse_mode=ParseMode.HTML)
...
def set_about_bio(update: Update, context: CallbackContext):
    ...
    if message.reply_to_message:
        ...
        text = message.text
        bio = text.split(
            None, 1)  # use python's maxsplit to only remove the cmd, hence keeping newlines.
