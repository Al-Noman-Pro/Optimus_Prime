from Optimus_Prime.modules.helper_funcs.telethn import IMMUNE_USERS, telethn
from Optimus_Prime import SUDO_USERS
...
async def Optimus_Prime_is_admin(chat_id: int):
    status = False
    Optimus_Prime = await telethn.get_me()
    async for user in telethn.iter_participants(
        chat_id, filter=ChannelParticipantsAdmins):
        
        if Optimus_Prime.id == user.id:
            status = True
            break
    return status
